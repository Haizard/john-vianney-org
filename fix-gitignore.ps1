...
# 1. Modify .gitignore to allow the build directory
$gitignorePath = ".gitignore"
if (Test-Path $gitignorePath) {
    $gitignoreContent = Get-Content -Path $gitignorePath -Raw
    
    # Remove or comment out the line that ignores the build directory
    $gitignoreContent = $gitignoreContent -replace "(?m)^build/?$", "# build"
    $gitignoreContent = $gitignoreContent -replace "(?m)^/build/?$", "# /build"
    $gitignoreContent = $gitignoreContent -replace "(?m)^frontend/school-frontend-app/build/?$", "# frontend/school-frontend-app/build"
    
    Set-Content -Path $gitignorePath -Value $gitignoreContent
    
    Write-Host ".gitignore modified to allow the build directory."
} else {
    Write-Host ".gitignore not found. Creating a new one."
...
"@
    ...
    Set-Content -Path $gitignorePath -Value $newGitignoreContent
    
    Write-Host "New .gitignore created that allows the build directory."
}

# 2. Create the build directory again
$buildDir = "frontend/school-frontend-app/build"
if (-not (Test-Path $buildDir)) {
    New-Item -ItemType Directory -Path $buildDir -Force
}

# 3. Create a custom index.html that works without React
$customIndexHtml = @'
<!DOCTYPE html>
<html lang="en">
<head>
...
# 4. Create a _redirects file for client-side routing
$redirectsContent = "/* /index.html 200"
Set-Content -Path "$buildDir/_redirects" -Value $redirectsContent

# 5. Create a simple favicon.ico (1x1 transparent pixel)
$faviconBuffer = [byte[]]@(
...
# 6. Force add the build directory to git
git add -f "$buildDir/index.html" "$buildDir/_redirects" "$buildDir/favicon.ico" ".gitignore"

# 7. Commit the changes
git commit -m "Force add build directory for direct Vercel deployment"

# 8. Push the changes to GitHub
git push

Write-Host "Build directory forced added to git and pushed to GitHub."
Write-Host "Vercel should now be able to deploy the custom build directly."
