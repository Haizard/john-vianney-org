...
# 5. Update vercel.json to use the custom build
$vercelJsonContent = @'
{
  "version": 2,
  "buildCommand": null,
...
# 6. Update package.json to skip the build process
$packageJsonPath = "package.json"
$packageJson = Get-Content -Path $packageJsonPath -Raw | ConvertFrom-Json

# Remove the build script to skip the build process
if ($packageJson.scripts.PSObject.Properties["build"]) {
    $packageJson.scripts.PSObject.Properties.Remove("build")
}
...
git add "$buildDir/index.html" "$buildDir/_redirects" "$buildDir/favicon.ico" "vercel.json" "package.json"

# 8. Commit the changes
git commit -m "Add direct custom build for Vercel deployment"

# 9. Push the changes to GitHub
git push

Write-Host "Direct custom build for Vercel deployment pushed to GitHub."
Write-Host "This bypasses the React build process entirely and provides a working solution immediately."
Write-Host "Vercel will deploy the custom build directly without running any build commands."
