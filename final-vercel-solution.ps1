...
# 6. Create a .gitignore-backup file
$gitignorePath = ".gitignore"
if (Test-Path $gitignorePath) {
    Copy-Item -Path $gitignorePath -Destination "$gitignorePath-backup"
}

# 7. Create a new .gitignore file that doesn't ignore the build directory
$newGitignoreContent = @"
...
# Explicitly don't ignore the build directory
...
git add -f "$buildDir/index.html" "$buildDir/_redirects" "$buildDir/favicon.ico" "vercel.json" ".gitignore"

# 9. Commit the changes
git commit -m "Final solution for Vercel deployment with custom build"

# 10. Push the changes to GitHub
git push

Write-Host "Final solution for Vercel deployment pushed to GitHub."
