/**
 * useOLevelReport Hook
 * 
 * Custom hook for managing O-Level report data with proper loading states,
...
/**
 * Hook for fetching and managing O-Level student report data
 ...
 * @param {number} options.refreshKey - Key to trigger refresh
 * @returns {Object} - Report data, loading state, error state, and fetch function
 */
const useOLevelReport = ({ studentId, examId, autoFetch = true, refreshKey = 0 }) => {
  ...
  const fetchReport = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      // Fetch data from API
      const data = await reportService.fetchOLevelStudentReport(studentId, examId, {
        forceRefresh,
        signal
      });
  ...
  }, [autoFetch, studentId, examId, fetchReport, refreshKey]);
...
export default useOLevelReport;
